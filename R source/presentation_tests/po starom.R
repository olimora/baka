...
for (i.fve in fve) {
  ...
  for (i.day in 1:nrow(chosen_days)) {
    ...
    for (x in 1:nrow(potencial)) {
      ...
      diff <- append(diff, ret)
    }

    train_set <- arrange(potencial, diff)[1:tm_velkost,]
    ...
    predic <-predict(forest, dayd, type="response", norm.votes=TRUE)
    # varImpPlot(forest)
    
    i.vec <- i.vec + 1
    # actual[i.vec] <- dayd[['praca']] 
    # output[i.vec] <- predic
    actual <- append(actual, dayd[['praca']])
    output <- append(output, predic)
