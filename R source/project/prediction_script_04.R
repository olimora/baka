...
prog.baseAll <- length(ll) * length(tren_velk_v) * length(e.neural_algorithm) * length(th_v) * vzorka

for (layers_i in 1:length(ll)) {
  e$neural_layers <- ll[layers_i]
  e.neural_layers <- unq(e$neural_layers)
  ...
  for (tren_velk_i in tren_velk_v) {
    ...
    for (neur_alg_i in e.neural_algorithm) {
      e$neural_algorithm <- neur_alg_i
      
      for (th_i in th_v) {
        e$neural_threshold <- th_i 
        e.neural_threshold <- th_i
        ...
        for (fve in 1:length(e.fve)) { #length(e.fve)
          ...
          for (i in 1:3) { #t.all_days_count
            ...
            if (prog.print_perc_all >= prog.printed + prog.diff) {
              ...
              print(sprintf("Neural: %03.2f%s all, p: %00000.d/%00000.d, Estimated time: %s, Actual: %s",
