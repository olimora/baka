...
INSERT INTO t_produkcia_den (datum, vykon, praca, fve)
	(SELECT datum, vykon, praca, fve FROM
		(select to_date(to_char(cas, 'YYYY-MM-DD'),'YYYY-MM-DD') datum, sum(vykon) vykon, sum(praca) praca,
...
INSERT INTO t_produkcia_hodina (cas, vykon, praca, produkcia_den)
	(SELECT cas, vykon, praca, produkcia_den FROM
		(select group_by_cas cas, sum(vykon) vykon, sum(praca) praca, in_fve, group_by_cas,
