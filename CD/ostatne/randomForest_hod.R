...
e.ntree <- c(700)
e.mtry <- c(3)
e.pod_gho <- c(210, 220, 230)[2]
e.pod_obl <- c(80, 85, 90)[2]
e.pod_tep <- c(30)
e.pod_vie <- c(5)
e.pod_dlz <- c(50)
obl_napk <- T
...

{
  ...
  e.den_hod <- "hod"
  formula <- unq("praca~gho+oblacnost+teplota+vietor+dlzkadna+elev") 
  in_gho <- TRUE
  in_obl <- TRUE
  in_tep <- TRUE
  in_vie <- TRUE
  ...
  
  {
    ...
    e.select_chosen_hours <- " SELECT cas, praca, gho, (100 - oblacnost) oblacnost, (teplota + 100) teplota, vietor, dlzkadna, elev 
    FROM v_data WHERE fve = %d AND datum = '%s' ORDER BY cas "
    
    e.tm_select_hod <- " SELECT cas, praca, gho, (100 - oblacnost) oblacnost, (teplota + 100) teplota, vietor, dlzkadna, elev 
    FROM v_data WHERE fve = %d AND datum IN ('%s') ORDER BY cas "
    
    e.tm_select <- " SELECT datum, sum(praca) praca, sum(gho) gho, sum(100-oblacnost) oblacnost, 
    sum(100 + teplota) teplota, sum(vietor) vietor, max(dlzkadna) dlzkadna
    FROM v_data WHERE fve = %d GROUP BY datum ORDER BY datum"

    e.tm_opis <- " %d najpodobnejsich dni podla" %s% 
              ...
              for (i.ntree in e.ntree) { 
                ...
                for (i.mtry in e.mtry) {
                  ...
                  for (i in 1:t.chosen_days_c) { #t.chosen_days_c
                    ...
                    t.hours <- dbGetQuery(db.con, sprintf(e.tm_select_hod, i.fve, paste0(t.train_set[['datum']], collapse = "', '")))

                    t.chosen_hours <- dbGetQuery(db.con, sprintf(e.select_chosen_hours, i.fve, t.chosen_one[['datum']]))

                    #actual <- vector.add(actual, t.chosen_one[1,'praca'])
                    actual <- append(actual, sum(t.chosen_hours[['praca']]))
                    
                    forest <- randomForest(formula, data=t.hours, ntree = i.ntree, mtry = i.mtry, importance=TRUE, proximity=TRUE)
                    output_hod <- predict(forest, t.chosen_hours[,3:ncol(t.chosen_hours)], type="response", norm.votes=TRUE)
