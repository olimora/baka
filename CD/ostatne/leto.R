...
write_results <- TRUE
dataset <- c("v_data_all", "v_data_120", "v_data", "v_data_vz", "v_data_vz_120")[2:5]
fve <- c(1, 2, 3)
tm_velkost <- c(30)
f_ntree <- c(500)
f_mtry <- c(2)
...
settings <- expand.grid(dataset = dataset, velkost = tm_velkost, ntree = f_ntree, 
                        mtry = f_mtry, nodesize = f_nodesize, pod_gho = pod_gho, 
                        pod_obl = pod_obl, pod_tep = pod_tep, pod_vie = pod_vie,
                        pod_vlh = pod_vlh, pod_dlz = pod_dlz, pod_ele = pod_ele, 
                        ele_res = ele_res)
...
for (i.dataset in dataset) {
  ...
  (select cas, fve from %s where fve IN (%s)
...
for (i.sett in 1:prog.opsAll) {
  ...
  for (i.fve in fve) {
    ...
    fve_output <- parSapply(cl, 1:nrow(chosen_hours), function(y) {
      ...
      #potencial <- potencial[potencial[,'datum'] > as.Date("") ,]
